...
data = pd.read_csv("../../data/ESG_split/ESG_label_token.csv")
##############
...
#Label
label_to_int = {}
for i, item in enumerate(data['keyword'].unique()):
    label_to_int[item] = i


data['keyword'] = data['keyword'].apply(lambda x : label_to_int[x])
data = data[['token', 'keyword']]
...
class BERTDataset(Dataset):
    def __init__(self, dataset,bert_tokenizer, max_len,
        ...
        self.labels = [np.int32(dataset.iloc[i]['keyword']) for i in range(len(dataset))]
...
class BERTClassifier(nn.Module):
    def __init__(self,
                 bert,
                 hidden_size = 768,
                 num_classes = len(dataset_train['keyword'].unique()),   ##클래스 수 조정##
                 dr_rate = None,
                 params=None):
...
torch.save(model, PATH + 'KoBERT_0217_e100_py.pt')  # 전체 모델 저장
torch.save(model.state_dict(), PATH + 'Kobert_0217_e100_py_state_dict.pt')  # 모델 객체의 state_dict 저장
torch.save({
